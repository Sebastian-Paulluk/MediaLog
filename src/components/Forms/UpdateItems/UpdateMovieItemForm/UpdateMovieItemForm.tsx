...
export const UpdateMovieItemForm: React.FC<UpdateMovieItemFormTypes> = ({
	item,
	onClose,
	onSubmit,
}) => {
	...
	const [notesTextCopiedVisible, setNotesTextCopiedVisible] =
	...
	const hasDataItemChanged = (
		currentData: Partial<MoviesItemTypes>,
		originalItem: ItemTypes
	): boolean => {
		if (isMoviesItem(originalItem)) {
			return (
				currentData.name !== originalItem.name ||
		...
		<form onSubmit={handleSubmit} className="update-movie-item-form form">
			...
			<div className="form-body">
				...
				<div className="field-container">
					<label htmlFor="fromMinute" className="form__label">
						From minute
					</label>
					<Counter
						min={0}
				...
				<div className="field-container from-min-container">
					<div className="notes-label">
						...
						<p
							className={`copy-notes-text ${
								notesTextCopiedVisible ? 'visible' : ''
							}`}
						>
							Copied to clipboard
						</p>
						<button
							className={`copy-notes-button ${
								!movieItemData.notes ? 'disabled' : ''
							}`}
							onClick={handleCopyNotes}
						>
							<img
								src={copyImg}
								className="copy-notes-button__img"
								alt="copy-img"
							/>
						</button>
					</div>
