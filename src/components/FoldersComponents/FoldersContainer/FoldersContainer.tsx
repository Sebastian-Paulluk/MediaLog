...
import {
	...
	deleteCategory,
	deleteFolder,
	deleteFoldersByCaregoryId,
	deleteItemsByCategoryId,
	deleteItemsByFolderId,
	updateCategory,
...
export const FoldersContainer: React.FC<FoldersContainerTypes> = ({
	category,
	openFoldersMenu,
	setOpenFoldersMenu,
}) => {
	const { getFoldersByCategoryId, getItemsByCategoryIdInRoot, folders } =
		useDataContext();
	...
	const [folderContainerState, setFolderContainerState] =
		useState<string>('');
	const itemsInRootOfCategory = category.id
		? getItemsByCategoryIdInRoot(category.id).length
		: 0;
	...
	}, [folders, folderId, category.id, getFoldersByCategoryId, getFolderById]);

	const getFolderContainerState = useCallback(() => {
		if (windowWidth > 1500) {
		...
		return '';
	}, [windowWidth, openFoldersMenu, setOpenFoldersMenu]);

	useEffect(() => {
		const state = getFolderContainerState();
		setFolderContainerState(state);
	}, [getFolderContainerState, openFoldersMenu, windowWidth]);
	...
	return (
		<div className={`folders-content ${folderContainerState}`}>
			<div className="folders-content__title">Folders</div>

			<div
				className={`folders-content__root-folder ${
					!activeFolder ? 'active' : ''
				}`}
				onClick={handleRootFolderClick}
			>
				<div
					className={`folders-content__root-folder__line ${
						localFolders.length > 0 ? '' : 'no-folders'
					}`}
				/>
