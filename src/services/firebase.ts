...
export const getUserData = async (uid?: string) => {
	try {
		...
		if (!userUid) {
			console.warn(
				'No se proporcionó un UID válido ni hay usuario autenticado.'
			);
			return null;
		}
...
export const updateUser = async (
	userId: string,
	updatedUserData: Partial<UserTypes>
) => {
	try {
...
export const createCategory = async (category: CategoryTypes) => {
	...
	const categoriesDoc = await addDoc(
		categoriesColletionRef,
		categoryWithUserId
	);
...
export const updateCategory = async (
	categoryId: string,
	category: Partial<CategoryTypes>
) => {
	try {
...
export const updateItem = async (
	itemId: string,
	updatedItemData: Partial<ItemTypes>
) => {
	try {
